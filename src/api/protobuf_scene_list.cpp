...
void SceneList::to_msg_string(std::string &out_string) {
  ...
  for (int a = 0; a < num_return_scenes; a++) {
    ...
    // Name & Key
    std::string key = get_scene(a)->get_key();
    obj_logging->info("Scene:To Proto message Called on object");
    obj_logging->info(key);
    if (!(key.empty())) {
      scn->set_key(key);
    }
    std::string name = get_scene(a)->get_name();
    if (!(name.empty())) {
      scn->set_name(name);
    }

    // Lat/long
    scn->set_latitude(get_scene(a)->get_latitude());
    scn->set_longitude(get_scene(a)->get_longitude());

    // distance
    if (data[a]->get_distance() >= 0.0) {
      scn->set_distance(get_scene(a)->get_distance());
    }
    ...
    if (get_scene(a)->has_transform()) {
      ...
      proto_translation->set_x(get_scene(a)->get_scene_transform()->translation(0));
      proto_translation->set_y(get_scene(a)->get_scene_transform()->translation(1));
      proto_translation->set_z(get_scene(a)->get_scene_transform()->translation(2));
      proto_rot->set_x(get_scene(a)->get_scene_transform()->rotation(0));
      proto_rot->set_y(get_scene(a)->get_scene_transform()->rotation(1));
      proto_rot->set_z(get_scene(a)->get_scene_transform()->rotation(2));
    }
    ...
    for (int b = 0; b < get_scene(a)->num_devices(); b++) {
      ...
      std::string key = get_scene(a)->get_device(b)->get_key();
      if (!(key.empty())) {
        ud->set_key(key);
      }
      ...
      if (get_scene(a)->get_device(b)->has_transform()) {
        ...
        protoScene::SceneList_Vertex3 *proto_translation = \
          trn->mutable_translation();
        protoScene::SceneList_Vertex3 *proto_rot = trn->mutable_rotation();
        proto_translation->set_x(get_scene(a)->get_device(b)->\
          get_transform()->translation(0));
        proto_translation->set_y(get_scene(a)->get_device(b)->\
          get_transform()->translation(1));
        proto_translation->set_z(get_scene(a)->get_device(b)->\
          get_transform()->translation(2));
        proto_rot->set_x(get_scene(a)->get_device(b)->get_transform()->rotation(0));
        proto_rot->set_y(get_scene(a)->get_device(b)->get_transform()->rotation(1));
        proto_rot->set_z(get_scene(a)->get_device(b)->get_transform()->rotation(2));
      }
    }
  }
