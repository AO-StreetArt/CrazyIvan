...
ProcessResult* MessageProcessor::process_create_message(Scene *obj_msg) {
  ...
  if (obj_msg->num_scenes() > 0) {
    ...
    if (!(obj_msg->get_scene(0)->get_key().empty())) {
      ...
      key_param = \
        neo_factory->get_neo4j_query_parameter(\
...
ProcessResult* MessageProcessor::process_registration_message(Scene *obj_msg) {
  ...
  if (current_err_code == NO_ERROR) {
    if (!(does_scene_exist)) {
      ...
      try {
        already_registered = \
          qh->is_ud_registered(obj_msg->get_scene(0)->get_key(), \
  ...
  if (current_err_code == NO_ERROR) {
    try {
      registered_scenes = \
        qh->get_registrations(obj_msg->get_scene(0)->get_device(0)->get_key());
    }
    catch (std::exception& e) {
  ...
  if (!(is_first_device) && does_scene_exist && !(already_registered) \
    ...
    processor_logging->debug("Attempting to find UD Transform");
  ...
  build_string_response(SCENE_ENTER, current_err_code, \
    ...
    obj_msg->get_scene(0)->get_key(), config->get_formattype());
