...
SceneData::SceneData(const SceneData& sd) {
  ...
  for (int j = 0; j < sd.num_devices(); j++) {
    ...
    UserDevice *new_dev = \
      new UserDevice (sd.get_device(j)->get_key(), new_tran);
    ...
    for (int b = 0; b < data[a]->num_devices(); b++) {
      ...
      if (data[a]->get_device(b)->has_transform()) {
        ...
        protoScene::SceneList_Vertex3 *proto_translation = \
          trn->mutable_translation();
        protoScene::SceneList_Vertex3 *proto_rot = trn->mutable_rotation();
        proto_translation->set_x(data[a]->get_device(b)->\
          get_transform()->translation(0));
        proto_translation->set_y(data[a]->get_device(b)->\
          get_transform()->translation(1));
        proto_translation->set_z(data[a]->get_device(b)->\
          ...
          if (itr.HasMember("devices")) {
            ...
            for (auto& device_itr : device_list_val.GetArray()) {
              ...
              if (ud_key_iter != device_itr.MemberEnd()) {
                ...
                UserDevice *new_device = \
                new UserDevice(ud_key_iter->value.GetString());
  ...
  std::string Scene::to_json() {
    ...
    for (int a = 0; a < num_scenes; a++) {
      ...
      if (!(data[a]->get_key().empty())) {
        ...
        writer.Key("key");
        writer.String(data[a]->get_key().c_str(), \
        (rapidjson::SizeType)data[a]->get_key().length());
      }
      // Add the name
      if (!(data[a]->get_name().empty())) {
        writer.Key("name");
        writer.String(data[a]->get_name().c_str(), \
        (rapidjson::SizeType)data[a]->get_name().length());
      }
      // Add the latitude
      ...
      for (int b = 0; b < data[a]->num_devices(); b++) {
        ...
        if (!(data[a]->get_device(b)->get_key().empty())) {
          writer.Key("key");
          writer.String(data[a]->get_device(b)->get_key().c_str(), \
          (rapidjson::SizeType)data[a]->get_device(b)->get_key().length());
        }
        ...
        if (data[a]->get_device(b)->has_transform()) {
          ...
          if (data[a]->get_device(b)->get_transform()->has_translation()) {
            ...
            writer.Key("translation");
            writer.StartArray();
            writer.Double(data[a]->get_device(b)->\
            get_transform()->translation(0));
            writer.Double(data[a]->get_device(b)->\
            get_transform()->translation(1));
            writer.Double(data[a]->get_device(b)->\
